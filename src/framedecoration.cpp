...
void FrameDecoration::render(const FrameDecorationData& data, bool isFocused) {
    ...
    if (isFocused) {
        Ewmh::get().ewmh_set_window_opacity(window, settings->frame_active_opacity()/100.0);
    } else {
        Ewmh::get().ewmh_set_window_opacity(window, settings->frame_normal_opacity()/100.0);
    }
